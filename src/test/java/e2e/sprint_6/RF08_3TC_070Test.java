...
/**
 * Sprint 6 - RF08.3
 * TC-070: Validar que, al marcar la revisión como No Conforme – Rechazo Definitivo,
 *         el estado pase a Rechazo Definitivo / Rechazada (según etiqueta del sistema).
 *
 * Flujo (idéntico a TC-069):
 * - Login
 * - Revisión Documental
 * - (si existe) Filtrar Estado=Enviada
 * - Iniciar Revisión
 * - Revisar -> Modal criterios -> Conforme=Sí -> Guardar y Definir Conformidad
 * - Click No Conforme (btn-danger btn-md)
 * - Modal: Click No Conforme (btn-danger)
 * - Modal "Decisión Final": llenar textarea
 * - Click "Rechazo Definitivo" (btn-danger)
 * - Validar estado final (actualiza selector si tu UI lo muestra en una columna/badge específico)
 */
class RF08_3TC_070Test {
    ...
    // ===== etiqueta esperada del estado (ajusta si tu sistema usa otra palabra) =====
    private static final String ESTADO_RECHAZO_EXPECT = "rechazo definitivo"; // o "rechazada"
    ...
    private void login(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF083_TC070_01_login_ok");
    }
    ...
    private void irARevisionDocumental(WebDriver d, WebDriverWait w) {
        ...
        waitDocumentReady(d);
        screenshot(d, "S6_RF083_TC070_02_modulo_revision_documental");
    }
    ...
    private void filtrarEstadoEnviadaSiExiste(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF083_TC070_03_estado_enviada");
        ...
        screenshot(d, "S6_RF083_TC070_04_filtrado");
    }
    ...
    private void iniciarRevision(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF083_TC070_05_comprobacion_documentos");
    }
    ...
    private void abrirModalCriteriosConRevisar(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF083_TC070_06_modal_criterios");
    }
    ...
    private void completarCriteriosYGuardar(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF083_TC070_07_criterios_llenos");
        ...
        screenshot(d, "S6_RF083_TC070_08_click_guardar_definir");
    ...
    // ===================== TC-070 (NO CONFORME - RECHAZO DEFINITIVO) =====================
    ...
    private void marcarNoConforme(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF083_TC070_09_no_conforme_main_click");
    }
    ...
    private void confirmarNoConformeModal(WebDriver d, WebDriverWait w) {
        ...
        w.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".modal.show, .modal-dialog, .modal-content")));
        screenshot(d, "S6_RF083_TC070_10_no_conforme_modal_click");
    }

    private void decisionFinalRechazoDefinitivo(WebDriver d, WebDriverWait w) {
        log("Decisión Final: llenar textarea y click Rechazo Definitivo");
        ...
        scrollCenter(d, txt);
        txt.clear();
        txt.sendKeys("TC-070 - Motivo general automático (rechazo definitivo)");

        screenshot(d, "S6_RF083_TC070_11_textarea_llena");

        // <button type="button" class="btn btn-danger">Rechazo Definitivo</button>
      // Click "Rechazo Definitivo"
By btnRechazo = By.xpath("//div[contains(@class,'modal') and contains(@class,'show')]//button[@type='button' and contains(@class,'btn-danger') and normalize-space()='Rechazo Definitivo']");
clickRobusto(d, w, btnRechazo);

screenshot(d, "S6_RF083_TC070_12_click_rechazo_definitivo");

// ✅ Aquí: si salen los rojos (No autorizado / No se pudo actualizar...), el test FALLA
failSiHayToastError(d, w, "S6_RF083_TC070_12A");

    }

    private boolean estadoEsRechazoDefinitivoEnPantalla(WebDriver d) {
        // Busca texto "Rechazo Definitivo" o "Rechazada" en elementos visibles.
        try {
            List<WebElement> hits = d.findElements(By.xpath(
                "//*[self::td or self::span or self::div or self::p or self::small]" +
                "[contains(translate(normalize-space(.),'RECHAZO DEFINITIVO','rechazo definitivo'),'rechazo definitivo')" +
                " or contains(translate(normalize-space(.),'RECHAZADA','rechazada'),'rechazada')]"
            ));
    ...
    private void validarCambioARechazoDefinitivo(WebDriver d, WebDriverWait w) {
        log("Validar que el estado cambie a 'Rechazo Definitivo' (o 'Rechazada').");

        boolean rechazo = false;
        try {
            rechazo = new WebDriverWait(d, Duration.ofSeconds(10)).until(x -> estadoEsRechazoDefinitivoEnPantalla(x));
        } catch (Exception ignore) {
            rechazo = estadoEsRechazoDefinitivoEnPantalla(d);
        }

        screenshot(d, "S6_RF083_TC070_13_validacion_estado");

        assertTrue(rechazo,
            "❌ TC-070 FALLÓ: El estado NO cambió a 'Rechazo Definitivo'/'Rechazada' tras 'Rechazo Definitivo'.");
    }


    private void failSiHayToastError(WebDriver d, WebDriverWait w, String shotName) {
    // Espera breve por si el toast tarda en salir
    try { Thread.sleep(600); } catch (InterruptedException ignore) {}

    // XPaths flexibles para “tarjetas/toasts” tipo bootstrap/react
    ...
    try {
        // Si aparece, lo consideramos FAIL (porque en negocio es error aunque el click haya funcionado)
    ...
    // ===================== TEST =====================
    @Test
    void RF083_TC070_MarcarNoConforme_RechazoDefinitivo_DebeCambiarEstado() {
        ...
        try {
            ...
            decisionFinalRechazoDefinitivo(driver, wait);

            validarCambioARechazoDefinitivo(driver, wait);

            System.out.println("✅ TC-070 OK: estado cambió a Rechazo Definitivo/Rechazada.");

        } catch (Exception e) {
            screenshot(driver, "S6_RF083_TC070_ERROR");
            throw e;
        } finally {
            // driver.quit();
        }
