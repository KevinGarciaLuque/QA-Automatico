...
/**
 * Sprint 6 - RF08.6
 * TC-133: Validar existencia del botón/enlace "Ver Inspección" en el listado.
 *
 * Flujo:
 * 1) Login
 ...
 * 4) Consultar
 * 5) Verificar que en la primera fila exista "Ver Inspección" y darle click
 * 6) Esperar 3 segundos y FIN
 */
class RF08_6TC_133Test {
    ...
    private void login(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF086_TC133_01_login_ok");
    }
    ...
    private void irAConsultaParametrizada(WebDriver d, WebDriverWait w) {
        ...
        waitDocumentReady(d);
        screenshot(d, "S6_RF086_TC133_02_consulta_parametrizada");
    }
    ...
    private void aplicarFiltrosYConsultar(WebDriver d, WebDriverWait w) {
        ...
        screenshot(d, "S6_RF086_TC133_03_filtros_listos");

        clickConsultar(d, w);

        waitDocumentReady(d);
        screenshot(d, "S6_RF086_TC133_04_click_consultar");
    }

    private void clickVerInspeccionYFinalizar(WebDriver d, WebDriverWait w) {
        esperarAlMenosUnaFila(d);
        screenshot(d, "S6_RF086_TC133_05_resultados");

        // Buscar SOLO dentro de la primera fila y en la columna de acciones:
        // - puede ser <button> o <a>
        // - puede venir "Inspección" o "Inspeccion"
        By verInspeccionEnPrimeraFila = By.xpath(
            "//table//tbody/tr[1]//td[last()]//*[self::button or self::a]" +
            "[contains(normalize-space(.),'Ver Inspección') or contains(normalize-space(.),'Ver Inspeccion')]"
        );
        ...
        try {
            new WebDriverWait(d, Duration.ofSeconds(12))
                .until(ExpectedConditions.presenceOfElementLocated(verInspeccionEnPrimeraFila));
            encontrados = d.findElements(verInspeccionEnPrimeraFila);
        } catch (TimeoutException te) {
            screenshot(d, "S6_RF086_TC133_FAIL_no_btn_ver_inspeccion");
            throw new AssertionError("❌ TC-133 FALLÓ: No se encontró 'Ver Inspección' en la primera fila (acciones).", te);
        }

        assertTrue(!encontrados.isEmpty(),
            "❌ TC-133 FALLÓ: No se encontró el botón/enlace 'Ver Inspección' en el listado (columna Acciones).");

        // Click robusto (si cambia el DOM por render)
        clickConReintento(d, verInspeccionEnPrimeraFila, 3);

        screenshot(d, "S6_RF086_TC133_06_click_ver_inspeccion");
    ...
    // ===================== TEST =====================
    @Test
    void RF086_TC133_Validar_Existencia_Boton_Ver_Inspeccion_En_Listado() {
        ...
        try {
            ...
            clickVerInspeccionYFinalizar(driver, wait);

            screenshot(driver, "S6_RF086_TC133_OK");
            System.out.println("✅ TC-133 OK: Existe y se hizo click en 'Ver Inspección' (evidencia en screenshot).");

        } catch (TimeoutException e) {
            screenshot(driver, "S6_RF086_TC133_TIMEOUT");
            throw e;
        } catch (Exception e) {
            screenshot(driver, "S6_RF086_TC133_ERROR");
            throw e;
        } finally {
