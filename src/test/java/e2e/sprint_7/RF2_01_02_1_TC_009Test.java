 ...
 * TC-009: Validar que el botón Crear Solicitud esté habilitado cuando existan ≤10
 * solicitudes en estado “Iniciado/Iniciada”.
 *
 * Flujo:
 * Login -> Solicitudes -> Estado=Iniciada -> Buscar
 * -> Validar total aprox <= 10
 * -> Validar botón "+ Crear Solicitud" HABILITADO
 */
public class RF2_01_02_1_TC_009Test {
    ...
    // ================== TEST ==================
    @Test
    void RF201021_TC009_CrearSolicitud_Habilitado_SiMenorIgual10Iniciadas() throws InterruptedException {
        ...
        try {
            ...
            screenshot(driver, "S7_RF201021_TC009_01_login_ok");

            // ====== IR A LISTADO ======
            irARegistroSolicitudes(driver, wait, js);
            screenshot(driver, "S7_RF201021_TC009_02_listado");

            // ====== FILTRAR ESTADO INICIADA ======
            seleccionarEstadoIniciada(driver, wait);
            screenshot(driver, "S7_RF201021_TC009_03_estado_iniciada_set");

            // ====== BUSCAR ======
            clickBuscar(driver, wait, js);
            screenshot(driver, "S7_RF201021_TC009_04_resultados_busqueda");
            ...
            if (hayPagina2) {
                ...
                screenshot(driver, "S7_RF201021_TC009_05_pagina2_iniciadas");
            }

            int totalAprox = filasPag1 + filasPag2;

            // Precondición TC-009: total <= 10
            assumeTrue(totalAprox <= 10,
                "⏭️ TC-009 SKIPPED: El ambiente tiene más de 10 solicitudes 'Iniciada' (total aprox=" + totalAprox + ").");

            // Volver a página 1 para validar el botón en la vista principal
            try {
            ...
            boolean habilitado = isButtonEnabled(driver, crearSolicitudBy);
            screenshot(driver, "S7_RF201021_TC009_06_crear_solicitud_estado");

            assertTrue(habilitado,
                "❌ Regla incumplida: Con ≤10 solicitudes 'Iniciada' (total aprox=" + totalAprox + "), " +
                "el botón 'Crear Solicitud' debería estar HABILITADO, pero está DESHABILITADO.");

            System.out.println("✅ TC-009 OK: total 'Iniciada' aprox=" + totalAprox + " => botón Crear Solicitud HABILITADO.");

        } catch (TimeoutException te) {
            screenshot(driver, "S7_RF201021_TC009_TIMEOUT");
            throw te;

        } catch (NoSuchElementException ne) {
            screenshot(driver, "S7_RF201021_TC009_NO_SUCH_ELEMENT");
            throw ne;
